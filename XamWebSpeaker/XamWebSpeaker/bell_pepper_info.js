...
gBellPepperInfo.IsValid = function (dom1) {
    var st, rec1, rec2, i1; rec1 = this.GetLoc(dom1);
...
gBellPepperInfo.FirstDom = function (dom1) {
    var str1, rec1; rec1 = this.IsValid(dom1);
...
gBellPepperInfo.NextDom = function (dom1) {
    var dom2, dom3, dom4; dom2 = dom1.nextSibling;
...
gBellPepperInfo.SpeakSent = function (str1) {
    var len1, k1, v1, n;
    if (bell_pepper_info_ReadPhrase() == 1) {
        n = 7; len1 = str1.length;
        if (30 < len1) {
            v1 = this.Reg.exec(str1.substr(n));
            if (v1 != null) {
                k1 = n + v1.index;
                if (k1 != n + -1 && n < len1 - k1) {
                    k1++; this.Remain = str1.substr(k1);
                    bell_pepper_info_Speak(str1.substr(0, k1));
                    return;
                }
            }
        }
...
gBellPepperInfo.SpeakDom2 = function (rem_str, dom1) { var str1, str2, dom2, dom3, k1, rec1, i; this.Remain = null; dom2 = dom1; str1 = null; rec1 = null; for (i = 0; ; i++) { if (i == 0 && rem_str != null) { str2 = rem_str; } else { str2 = this.GetSpeakText(dom2); } if (str2 != null && 0 < str2.length) { rec1 = this.GetDomRect(rec1, dom2); if (str1 == null) { str1 = ''; } else { str1 = str1 + ' '; } k1 = bell_pepper_info_SplitSent(str2); if (k1 != -1) { str1 = str1 + str2.substr(0, k1); this.Remain = str2.substr(k1); break; } str1 = str1 + str2; if (bell_pepper_info_MaxTextLen() < str1.length) { break; } } this.AllInline = true; dom3 = this.NextDom(dom2); if (dom3 == null || str1 != null && this.AllInline != true) { break; } dom2 = dom3; } if (str1 != null) { if (rec1 != null) { this.ShowMarkScroll(rec1); } this.CurDom = dom2; bell_pepper_info_Speak(str1); } };
gBellPepperInfo.SpeakDom = function (dom1) {
    var str1;
    switch (dom1.nodeName) {
        case '#text':
            this.SelectDom(dom1);
            this.SpeakSent(dom1.nodeValue);
            break;
        case 'SELECT': this.SelectDom(dom1); str1 = this.GetSelectText(dom1); if (str1 != null) { bell_pepper_info_Speak(str1); } break;
        case 'RUBY': this.SelectDom(dom1); str1 = this.GetRubyText(dom1); if (str1 != null) { bell_pepper_info_Speak(str1); } break;
    }
};
            ...
            if (dom1.hasChildNodes()) {
                ...
                for (var i = 0; i < children.length; i++) {
                    ...
                    if (rec1 == null) { }
                        ...
                        x1 = window.scrollX + rec1.left; y1 = window.scrollY + rec1.top; x2 = x1 + rec1.width; y2 = y1 + rec1.height;
...
gBellPepperInfo.PageFinished = function () {
    if (document.m_BellPepperInfo == undefined) {
        document.m_BellPepperInfo = gBellPepperInfo;
...
gBellPepperInfo.SpeakCur = function () {
    ...
    if (this.CurDom == null) { bell_pepper_info_Speak('empty document'); }
    else {
        if (bell_pepper_info_ReadPhrase() == 2) { this.SpeakDom2(null, this.CurDom); }
        else { this.SpeakDom(this.CurDom); }
    }
};
gBellPepperInfo.SpeakNext = function () {
    var k1;
    if (this.Remain != null) {
        if (bell_pepper_info_ReadPhrase() == 2) { this.SpeakDom2(this.Remain, this.CurDom); }
    ...
    if (this.CurDom == null) { bell_pepper_info_SpeakEnd('end of the page'); }
    else {
        if (bell_pepper_info_ReadPhrase() == 2) { this.SpeakDom2(null, this.CurDom); }
        else { this.SpeakDom(this.CurDom); }
    }
...
gBellPepperInfo.FastForward = function () { this.StopTimer(); if (this.CurDom == null || !this.ChkDom(this.CurDom)) { this.CurDom = this.FirstDom(document.body); } if (this.CurDom == null) { bell_pepper_info_Speak('empty document'); } else { this.StopTimer(); this.TimerId = setInterval(function () { gBellPepperInfo.FastForwardFnc(); }, 100); } };
gBellPepperInfo.Rewind = function () { this.StopTimer(); if (this.CurDom == null || !this.ChkDom(this.CurDom)) { this.CurDom = this.LastDom(document.body); } if (this.CurDom == null) { bell_pepper_info_Speak('empty document'); } else { this.StopTimer(); this.TimerId = setInterval(function () { gBellPepperInfo.RewindFnc(); }, 100); } };
...
gBellPepperInfo.GetTextAll = function () { try { bell_pepper_info_FileSaveAs(document.body.innerText); } catch (e) { } };
